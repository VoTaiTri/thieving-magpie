    ...
    lists.each_with_index do |link, num|
      begin
        ...
        if jobs_hash[:url].include? "hellowork"
          companies_hash[:raw_address] = parse_full_address company_table[3]
        else
          companies_hash[:raw_address] = parse_full_address work_table[8]
        end

        full_address = get_full_address companies_hash[:raw_address]
